...
def small_cover_check(path, number, cover_small, c_word, conf: config.Config, filepath, failed_folder):
    download_file_with_filename(
        cover_small, number + c_word + '-poster.jpg', path, conf, filepath, failed_folder)
    print('[+]Image Downloaded! ' + path +
          '/' + number + c_word + '-poster.jpg')


def create_folder(success_folder, location_rule, json_data, conf: config.Config):  # 创建文件夹
    title, studio, year, outline, runtime, director, actor_photo, release, number, cover, website, series, label = get_info(
        json_data)
    if len(location_rule) > 240:  # 新建成功输出文件夹
        path = success_folder + '/' + location_rule.replace(
            "'actor'", "'manypeople'", 3).replace("actor", "'manypeople'", 3)  # path为影片+元数据所在目录
    else:
...
def download_file_with_filename(url, filename, path, conf: config.Config, filepath, failed_folder):
    ...
    for i in range(retry_count):
        try:
            ...
            i += 1
            print('[-]Image Download :  Connect retry ' +
                  str(i) + '/' + str(retry_count))
...
def image_download(cover, number, c_word, path, conf: config.Config, filepath, failed_folder):
    ...
    for i in range(retry):
        if os.path.getsize(path + '/' + number + c_word + '-fanart.jpg') == 0:
            ...
            download_file_with_filename(
                cover, number + c_word + '-fanart.jpg', path, conf, filepath, failed_folder)
            continue
        else:
            break
    if os.path.getsize(path + '/' + number + c_word + '-fanart.jpg') == 0:
...
def print_files(path, c_word, naming_rule, part, cn_sub, json_data, filepath, failed_folder, tag, actor_list, liuchu):
    ...
    try:
        ...
        with open(path + "/" + number + part + c_word + ".nfo", "wt", encoding='UTF-8') as code:
            ...
            print("  <poster>" + number + c_word +
                  "-poster.jpg</poster>", file=code)
            print("  <thumb>" + number + c_word +
                  "-thumb.jpg</thumb>", file=code)
            print("  <fanart>" + number + c_word +
                  '-fanart.jpg' + "</fanart>", file=code)
            try:
...
def cutImage(imagecut, path, number, c_word):
    if imagecut == 1:  # 剪裁大封面
        ...
        shutil.copyfile(path + '/' + number + c_word + '-fanart.jpg',
                        path + '/' + number + c_word + '-poster.jpg')
        print('[+]Image Copyed!     ' + path +
              '/' + number + c_word + '-poster.jpg')


# 文件路径，番号，后缀，要移动至的位置
def paste_file_to_folder(filepath, path, number, c_word, conf: config.Config, work_folder):
    houzhui = str(re.search(
        '[.](iso|ISO|AVI|RMVB|WMV|MOV|MP4|MKV|FLV|TS|WEBM|avi|rmvb|wmv|mov|mp4|mkv|flv|ts|webm)$', filepath).group())
    ...
    try:
        ...
        if conf.soft_link():
            ...
            new_path = path + '/' + number + c_word + houzhui
            os.rename(filepath, new_path)
            print('[!]moved to ', new_path)  # todo fix 本部分 work_folder 逻辑有问题
        if os.path.exists(work_folder + '/' + number + c_word + '.srt'):  # 字幕移动
            os.rename(work_folder + '/' + number + c_word + '.srt',
                      path + '/' + number + c_word + '.srt')
            print('[+]Sub moved!')
        elif os.path.exists(work_folder + '/' + number + c_word + '.ssa'):
            os.rename(work_folder + '/' + number + c_word + '.ssa',
                      path + '/' + number + c_word + '.ssa')
            print('[+]Sub moved!')
        elif os.path.exists(work_folder + '/' + number + c_word + '.sub'):
            os.rename(work_folder + '/' + number + c_word + '.sub',
                      path + '/' + number + c_word + '.sub')
            print('[+]Sub moved!')
...
# 文件路径，番号，后缀，要移动至的位置
def paste_file_to_folder_mode2(filepath, path, multi_part, number, part, c_word, conf):
    if multi_part == 1:
        number += part  # 这时number会被附加上CD1后缀
    houzhui = str(re.search(
        '[.](AVI|RMVB|WMV|MOV|MP4|MKV|FLV|TS|WEBM|avi|rmvb|wmv|mov|mp4|mkv|flv|ts|webm|iso|ISO)$', filepath).group())
    ...
    try:
        ...
        if os.path.exists(number + '.srt'):  # 字幕移动
            os.rename(number + part + c_word + '.srt', path +
                      '/' + number + part + c_word + '.srt')
            print('[+]Sub moved!')
        elif os.path.exists(number + part + c_word + '.ass'):
            os.rename(number + part + c_word + '.ass', path +
                      '/' + number + part + c_word + '.ass')
            print('[+]Sub moved!')
        elif os.path.exists(number + part + c_word + '.sub'):
            os.rename(number + part + c_word + '.sub', path +
                      '/' + number + part + c_word + '.sub')
            print('[+]Sub moved!')
        print('[!]Success')
    except FileExistsError:
        print('[-]File Exists! Please check your movie!')
...
def core_main(file_path, number_th, conf: config.Config, work_folder):
    ...
    number = number_th
    # 定义番号             ##todo 空人名 判断为素人
    json_data = get_data_from_json(number, filepath, conf)
    ...
    if '-c.' in filepath or '-C.' in filepath or '中文' in filepath or '字幕' in filepath or '㊥' in filepath:  # dev中文  todo 可加入 config.ini
        cn_sub = '1'
        c_word = '-C'  # 中文字幕影片后缀
    if '流出' in filepath:
        liuchu = '流出'
    ...
    # 创建文件夹
    path = create_folder(conf.success_folder(),  json_data.get(
        'location_rule'), json_data, conf)
    ...
    if conf.main_mode() == 1:
        ...
        if imagecut == 3:
            small_cover_check(path, number,  json_data.get(
                'cover_small'), c_word, conf, filepath, conf.failed_folder())

        # creatFolder会返回番号路径
        image_download(json_data.get('cover'), number, c_word,
                       path, conf, filepath, conf.failed_folder())

        # 裁剪图
        cutImage(imagecut, path, number, c_word)

        # 打印文件
        print_files(path, c_word,  json_data.get('naming_rule'), part, cn_sub, json_data,
                    filepath, conf.failed_folder(), tag,  json_data.get('actor_list'), liuchu)
        ...
        paste_file_to_folder_mode2(
            filepath, path, multi_part, number, part, c_word, conf)
